...
| 参数名 | 类型 | 是否必填 | 描述 |
| ----- | ---- | ------ | ---- |
| text | `string` | 必填 | 文本内容，如果设置 `dangerousHTML` 为 `true` 则输出 HTML（危险行为，需要注意）|
| dangerousHTML ⚠️ | `boolean` | 选填 | 输出 `text` 文本为 HTML 形式展示 |
...
// 看板娘提示一段 HTML 内容
pio.message(`保罗的小窝有动态更新啦，点击 <a href="https://paul.ren/note" target="_blank">这里</a> 访问`, true);
```
