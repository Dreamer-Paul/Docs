...
答：在你模板的设置选项找到重载设置，通过本插件的 [重载方法](#重载函数（21-新增）) 进行重载。还有一种极端方法就是直接关闭 PJAX 功能。
...
> 从 2.4 版本开始，自定内容选择器和本功能合并，自定内容选择器的文章提示功能可参考本节点的 `custom` 方式实现。
...
如果你不通过插件的形式食用，或是要在插件里实现一些高级功能，即可参考本节点，配置看板娘启动器。
...
> 此功能在 2.2 版本中新增，但 2.3 及后期版本的编写方式有所改变。

只允许数组，数组内包含一个设置 `选择器` 和 `提示文字` 的对象，用于扩展看板娘的自定义提示内容。当鼠标移入指定的元素，将提示对应的内容。不填则不启用该功能。
